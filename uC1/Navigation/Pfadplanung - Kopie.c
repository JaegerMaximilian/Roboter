...
uint8_t PATH_DriveToAbsPos(int16_t vMax, point_t start, point_t ziel, uint8_t gegner)
{
	...
	robot = PATH_Get_ObstacleLimits(enemyRobot_1);
	PATH_AddItemToObstacleList(PATH_ENEMY_ROBOT_1, robot.xy.Xpos, robot.xy.Ypos, robot.XY.Xpos, robot.XY.Ypos);
 	robot = PATH_Get_ObstacleLimits(enemyRobot_2);
 	PATH_AddItemToObstacleList(PATH_ENEMY_ROBOT_2, robot.xy.Xpos, robot.xy.Ypos, robot.XY.Xpos, robot.XY.Ypos);
 	robot = PATH_Get_ObstacleLimits(smallRobot);
 	PATH_AddItemToObstacleList(PATH_SECOND_ROBOT, robot.xy.Xpos, robot.xy.Ypos, robot.XY.Xpos, robot.XY.Ypos);
 	
 	/* put a static obstacle to occupancy grid */
 	PATH_AddItemToObstacleList(PATH_STATIC_OBSTACLE, 15, 13, 24, 16);
 	/* put a static obstacle to occupancy grid */
 	PATH_AddItemToObstacleList(PATH_STATIC_OBSTACLE, 10, 12, 12, 14);
	...
	PATH_Set_ObstacleListInOccupancyGrid();
 	...
 	if (PATH_A_Star(startMP, zielMP))
 	{
		...
		/* make a list of all segments of tthe objects (vectors between the edges) */  
		PATH_SetSegmentList();
		...
		PATH_StoreRealPoints(zielMP, ziel);
...
matrixpoint_t PATH_FindNearestMatrixPoint(point_t pos)
{
	...
	gridPoint.Xpos = (uint8_t)((pos.Xpos - PATH_NON_TRAFFICLE_AREA) / PATH_GRID_RESOLUTION);
	gridPoint.Ypos = (uint8_t)((pos.Ypos - PATH_NON_TRAFFICLE_AREA) / PATH_GRID_RESOLUTION);
	
	/* limit grid-point in x */
	gridPoint.Xpos = ((gridPoint.Xpos > PATH_GRID_DIM_X) ? PATH_GRID_DIM_X : gridPoint.Xpos);
	gridPoint.Xpos = ((gridPoint.Xpos < 0) ? 0 : gridPoint.Xpos);

	/* limit grid-point in y */
	gridPoint.Ypos = ((gridPoint.Ypos > PATH_GRID_DIM_Y) ? PATH_GRID_DIM_Y : gridPoint.Ypos);
...
obstacle_t PATH_Get_ObstacleLimits(point_t obstacle)
{
	...
	if ((obstacle.Xpos > 0) && (obstacle.Ypos > 0))
	{
		...
		/* set lower limit */
		robot.xy.Xpos = (((obstacleInGrid.Xpos - 3) < 0) ? 0 : (obstacleInGrid.Xpos - 3));
		robot.xy.Ypos = (((obstacleInGrid.Ypos - 3) < 0) ? 0 : (obstacleInGrid.Ypos - 3));
		/* set upper limit */
		robot.XY.Xpos = (((obstacleInGrid.Xpos + 3) >= PATH_GRID_DIM_X) ? (PATH_GRID_DIM_X - 1) : (obstacleInGrid.Xpos + 3));
		robot.XY.Ypos = (((obstacleInGrid.Ypos + 3) >= PATH_GRID_DIM_Y) ? (PATH_GRID_DIM_Y - 1) : (obstacleInGrid.Ypos + 3));
	}
...
void PATH_StoreRealPoints(matrixpoint_t goal, point_t realGoal)
{
	...
	while ((path.finalList[i].Xpos != goal.Xpos) || (path.finalList[i].Ypos != goal.Ypos)) 
	{
		/* calculate the real position and store it to the point-list */
		path.pointList[i].Xpos = PATH_NON_TRAFFICLE_AREA + PATH_GRID_RESOLUTION * (int16_t)(path.finalList[i].Xpos);
		path.pointList[i].Ypos = PATH_NON_TRAFFICLE_AREA + PATH_GRID_RESOLUTION * (int16_t)(path.finalList[i].Ypos);
		i++;
	}
	/* store goal-point to final-list */
	path.pointList[i] = realGoal;
}
