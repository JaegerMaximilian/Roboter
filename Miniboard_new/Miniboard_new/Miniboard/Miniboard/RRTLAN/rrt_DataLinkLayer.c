  ...
  switch(usart->receivestate)
   {
     ...
     /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
       ...
       if(checksum == 0)
        ...
        {  
			 //uint8_t text[20];
			 //sprintf(text,"CS_ERROR %d - ",checksum);
			 //writeString_usart(&usartC1, text);
			 //for (uint8_t i = 0; i < (unsigned int)(usart->memtemp[FIRST_BLOCK - 1]) + (FIRST_BLOCK) + 2; i++)
			 //{
				 //sprintf(text,"%d.",*(usart->memptr));
				 //writeString_usart(&usartC1, text);
				 //(usart->memptr)++;
			 //}
			 //sprintf(text,"\r");
