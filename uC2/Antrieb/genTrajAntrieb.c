...
void genTrajAntrieb(void)
{
   ...
   {  
      ...
      paramOutputAntrieb.fPhin_1 = 0.0; 
		...
		if (setPos == 2)
		{
			paramOutputAntrieb.fX = (float)(siXPosition) / 10000.0;
			paramOutputAntrieb.fY = (float)(siYPosition) / 10000.0;
			paramOutputAntrieb.fPhi = (float)(siWinkel)  * M_PI / 1800.0;
...
//debugMsg(text);
						...
						else
						{
							...
							if (ii == 0)
							{
								vectorA[X_POS] = paramOutputAntrieb.fX - trayPoints[ii].x;	// startPoint[X] - supportPoint[X];
								vectorA[Y_POS] = paramOutputAntrieb.fY - trayPoints[ii].y;	// startPoint[Y] - supportPoint[Y];
								vectorB[X_POS] = trayPoints[ii+1].x - trayPoints[ii].x;		// finalPoint[X] - supportPoint[X];
								vectorB[Y_POS] = trayPoints[ii+1].y - trayPoints[ii].y;		// finalPoint[Y] - supportPoint[Y];
							}
							// all between
							else
							{
								vectorA[X_POS] = trayPoints[ii-1].x - trayPoints[ii].x;		// startPoint[X] - supportPoint[X];
								vectorA[Y_POS] = trayPoints[ii-1].y - trayPoints[ii].y;		// startPoint[Y] - supportPoint[Y];
								vectorB[X_POS] = trayPoints[ii+1].x - trayPoints[ii].x;		// finalPoint[X] - supportPoint[X];
								vectorB[Y_POS] = trayPoints[ii+1].y - trayPoints[ii].y;		// finalPoint[Y] - supportPoint[Y];
							}
...
//sprintf(text,"alpha:%3.1f;cT:%3.3f;A:%3.3f;s:%3.3f;v0:%3.3f;vs:%3.3f;T:%3.3f;a:%3.3f;b:%3.3f;k_w:%3.3f\r", (clothoidAlpha*180/M_PI), clothoidT, clothoidParameter, clothoidArcLength, clothoidVelocities[0], clothoidVelocities[1], T, alpha, beta, k_w);
//debugMsg(text);
...
//sprintf(text,"T:%3.3f;v0:%3.3f;vs:%3.3f;T:%3.3f;a:%3.3f;b:%3.3f;k_w:%3.3f\r",
...
// 	}		
   ...
   // **************************************************
   if((fabs(paramOutputAntrieb.fX - istX_) > Schleppfehler) || 
      (fabs(paramOutputAntrieb.fY - istY_) > Schleppfehler) || 
   ...
   paramOutputAntrieb.fVy = paramOutputAntrieb.fVn * sin(paramOutputAntrieb.fPhi - fPhi_2);
	...
	//{
		//if(zaehler++ >= 0)
		//{
			//sprintf(text,"%1.3f;%1.3f;%1.3f;%1.3f;%1.3f;%1.3f;%1.3f;%1.3f\r", paramOutputAntrieb.fX, paramOutputAntrieb.fY, paramOutputAntrieb.fVx, paramOutputAntrieb.fVy, paramOutputAntrieb.fPhi,paramOutputAntrieb.fW,global_t,paramComAntrieb.fT);
			//debugMsg(text);
			//zaehler = 0;
		//}
	//}	
